...
class Numbergame1 {
    public static void main(String args[]) {
        ...
        while (playagain && noofrounds < 3) {
            noofrounds++;
            randomno = r1.nextInt(1, 101); // Generate a new random number each round
            noofguess = 0; // Reset the number of guesses for each round
            guessiscorrect = false; // Reset the guess correctness for each round

            System.out.println("Round " + noofrounds + ": Guess a number between 1 and 100");

            while (noofguess < 3 && !guessiscorrect) {
                inputno = sc.nextInt();
                noofguess++;
            ...
            if (noofrounds < 3) { // Ask to play again if rounds are left
